...
def run_optimization(
    ...
    seed_candidate = {
        "instructions": json.dumps(INITIAL_INSTRUCTIONS)
    }
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Prompt optimization example using GEPA with pydantic-ai"
    )
    ...
    eval_parser.add_argument(
        "--expert", action="store_true", help="Use expert instructions"
    )

    # Optimize command
    opt_parser = subparsers.add_parser("optimize", help="Run optimization")
    opt_parser.add_argument(
        "--max-calls", type=int, default=50, help="Maximum metric calls"
    )
    opt_parser.add_argument(
        "--output", type=str, help="File to save optimized instructions"
    )
