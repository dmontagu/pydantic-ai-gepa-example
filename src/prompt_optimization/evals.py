    ...
    def evaluate(self, ctx: EvaluatorContext[TaskInput, ContactInfo, ContactCaseMetadata]) -> dict[str, Any]:
        ...
        for field in fields:
            ...
            if expected_val is not None:
                ...
                is_correct = expected_norm == output_norm or expected_norm in output_norm or output_norm in expected_norm
                if is_correct:
                    correct += 1
...
Alex"""
    ...
    Case(
        name="partial_info",
        inputs=TaskInput(
            text="Please contact support at help@service.io for assistance."
        ),
        expected_output=ContactInfo(
    ...
    Case(
        name="informal_intro",
        inputs=TaskInput(
            text="Hey! I'm Sam from StartupXYZ. Hit me up at sam@startupxyz.com or text 555-STARTUP"
        ),
        expected_output=ContactInfo(
            name="Sam",
